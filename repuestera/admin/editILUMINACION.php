...
// ID de categoría para iluminación
// (asegurate de que en la tabla categoria exista el registro con nombre_categoria = 'iluminacion')
$stmtCat = $pdo->prepare("SELECT id_categoria FROM categoria WHERE nombre_categoria = 'iluminacion'");
$stmtCat->execute();
$id_categoria = $stmtCat->fetchColumn();

if (!$id_categoria) {
    die("❌ No existe la categoría 'iluminacion' en la tabla categoria.");
}

// --- Eliminar imagen ---
if (isset($_GET['eliminar'])) {
    $id = intval($_GET['eliminar']);
    $stmt = $pdo->prepare("SELECT ruta_imagen FROM imagenes WHERE id_imagen = ? AND id_categoria = ?");
    ...
    $stmt = $pdo->prepare("DELETE FROM imagenes WHERE id_imagen = ? AND id_categoria = ?");
    $stmt->execute([$id, $id_categoria]);
...
$stmt = $pdo->prepare("SELECT * FROM imagenes WHERE id_categoria = ? ORDER BY id_imagen DESC");
...
<head>
<meta charset="utf-8">
<title>Editar Iluminación</title>
<style>
body {
...
<h1>🛠 Panel de Edición - Iluminación</h1>
...
<a href="/paginas/led.php" class="volver">← Volver a LED</a>
</body>
</html>